...
const BOT_NAME_FANCY = '💚SINETH-X-MD💚';
...
const config = {
  ...
  OTP_EXPIRY: 300000,
  WORK_TYPE: 'public',
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94781629812,94743387798',
  ...
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: '💚SINETH THISARA💚',
  IMAGE_PATH: 'https://files.catbox.moe/l74kdf.jpg',
  BOT_FOOTER: '> 𝙿𝙾𝚆𝙴𝚁𝙳 𝙱𝚈 💚SINETH-X-MD💚 ',
...
// ========== END WORK TYPE RESTRICTIONS ==========
    ...
    let botName = cfg.botName || '𝙰𝙻𝙾𝙽𝙴 𝚇 𝙼𝙳';
...
case 'gpt': {
  try {
    ...
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '💚SINETH-X-MD💚';
 ...
 case 'weather':
    try {
        ...
        const messages = {
            ...
            weather: (data) => `
*☘️ 💚SINETH-X-MD💚 𝐖eather 𝐑eport ☘️*
...
case 'fbd': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || '©Qᴜᴇᴇɴ 𝙳𝚄𝙻𝙰 𝙳𝙴𝚅';
...
case 'save': {
  try {
    ...
    if (quotedMsg.imageMessage || quotedMsg.videoMessage || quotedMsg.audioMessage || quotedMsg.documentMessage || quotedMsg.stickerMessage) {
      ...
      await socket.sendMessage(sender, { text: '🔥 *𝐒tatus 𝐒aved 𝐒uccessfully! 💚SINETH-X-MD💚*' }, { quoted: msg });

    } else if (quotedMsg.conversation || quotedMsg.extendedTextMessage) {
      const text = quotedMsg.conversation || quotedMsg.extendedTextMessage.text;
      await socket.sendMessage(saveChat, { text: `✅ *𝐒tatus 𝐒aved*\n\n${text}` });
      await socket.sendMessage(sender, { text: '🔥 *𝐓ext 𝐒tatus 𝐒aved 𝐒uccessfully!💚SINETH-X-MD💚*' }, { quoted: msg });
    } else {
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '💚SINETH-X-MD💚';
...
case 'creative': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || '©Qᴜᴇᴇɴ 𝙳𝚄𝙻𝙰 𝙳𝙴𝚅';
...
*👤 𝐍ame: SINETH,MADUSANKA*
*📞 𝐍umber: +94781629812,94783731694*

> 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝐁𝚈 💚SINETH-X-MD💚
`.trim();
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> *💚SINETH-X-MD💚*',
            buttons: buttons,
             headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TEL;type=CELL;type=VOICE;waid=94781629812\n' + // WhatsApp Number
      'TEL;type=CELL;type=VOICE;waid=94783731694\n' + // Second Number (Owner)
      'END:VCARD';
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '💚SINETH-X-MD💚'; // dynamic bot name
